...
### 3.40.3
- **Fix Gradle `project(":path")` undercount in Forma-style `deps(...)` blocks** — the wrapper-anchored regex `\b(\w+)\s*\(\s*project\s*\(` only matched the *first* `project(...)` per `deps(` block, silently dropping the rest. On real Forma projects this masked the majority of internal edges (e.g. `dependents` returning 0 when the truth was hundreds). Added a project-only fallback regex with a per-file `(module_id, dep_id)` HashSet to keep `dep_kind` from the wrapper-anchored match while catching every additional `project(...)` declaration. Regression test covers a `deps( aar(...) ) + deps( externals ) + deps( project(...), project(...), project(...) )` fixture
